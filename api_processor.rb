...
module ApiProcessor
  def self.call
    Application.logger.info "Start ApiProcessor#call at #{ Time.now }"

    #response = RestClient::Request.execute(
    #  method: :get,
    #  url: Application.settings["fake_server_url"]
    #)
    ...
    response = '[
      {
        ...
        "referrerName": "966634dc-0bf6-1ff7-f4b6-08000c95c670",
    ...
    all_info = JSON.parse(response)

    all_info.each do |v_info|
      v = Visit.new
      v.evid = v_info['referrerName']
      v.vendor_site_id = v_info['idSite']
      v.vendor_visit_id = v_info['idVisit']
      v.visit_ip = v_info['visitIp']
      v.vendor_visitor_id = v_info['visitorId']
      v.save

      v_info["actionDetails"].each_with_index do |pw_info, index|
        pw = PageView.new
        pw.visit_id = v.id
        pw.title = pw_info['pageTitle']
        pw.position = index
        pw.url = pw_info['url']
        pw.time_spent = pw_info['timeSpent']
        pw.timestamp = pw_info['timestamp']
        pw.save
      end
    end    

    Application.logger.info "End ApiProcessor#call at #{ Time.now }"
  end
end
